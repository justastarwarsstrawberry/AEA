...
function getRandomInt(min, max) {
    ...
    if (x===19) {
    const attachment = new Attachment('./resources/map/p6Dire_Straight-Large_6p_map.png');
    message.channel.send(attachment);
        message.reply('Dire Straight Large');

}
    if (x===20) {
    const attachment = new Attachment('./resources/map/PASSAGES_V2_6p_map.png');
    message.channel.send(attachment);
        message.reply('Passages v2');

}
    if (x===21) {
    const attachment = new Attachment('./resources/map/p6Gladiator_Island_6p_map.png');
    message.channel.send(attachment);
        message.reply('Gladiator Island');

}
if (x===22) {
    const attachment = new Attachment('./resources/map/p6Animate_Isle_6p_map.png');
    message.channel.send(attachment);
        message.reply('Animate Isle');

}
if (x===23) {
    const attachment = new Attachment('./resources/map/p6Lava_River_6p_map.png');
    message.channel.send(attachment);
        message.reply('Lava River');

}
if (x===24) {
    const attachment = new Attachment('./resources/map/p6Island_Hop_6p_map.png');
    message.channel.send(attachment);
